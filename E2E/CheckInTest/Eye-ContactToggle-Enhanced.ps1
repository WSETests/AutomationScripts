...
function EyeContact-Enhanced-Playlist($devPowStat, $token, $SPId) 
{   
    $startTime = Get-Date    
    $ErrorActionPreference='Stop'
    $scenarioName = "$devPowStat\EyeContactEnhanced"
    $logFile = "$devPowStat-EyeContactEnhanced.txt"
    
    $wsev2PolicyState = CheckWSEV2Policy 
    if($wsev2PolicyState -eq $false)
    {
       TestOutputMessage $scenarioName "Skipped" $startTime "EyeContact-Enhanced Not Supported"  
    }
    else
    {  
       ...
       try
	   {
           ...
           ToggleAIEffectsInSettingsApp -AFVal "Off" -PLVal "Off" -BBVal "Off" -BSVal "False" -BPVal "False" `
                                        -ECVal "On" -ECSVal "False" -ECEVal "True" -VFVal "Off" `
           ...
           Verifylogs $scenarioName "131072" $startTime
