...
function AutoFraming-Playlist($devPowStat, $token, $SPId) 
{   
    $startTime = Get-Date    
    $ErrorActionPreference='Stop'
    $scenarioName = "$devPowStat\AutomaticFraming"
    $logFile = "$devPowStat-AutomaticFraming.txt"
	...
	{   
        ...
        Write-Output "Entering ToggleAIEffectsInSettingsApp function to toggle Automatic framing On"
        ToggleAIEffectsInSettingsApp -AFVal "On" -PLVal "Off" -BBVal "Off" -BSVal "False" -BPVal "False" `
        ...
        Verifylogs $scenarioName "65536" $startTime 
