...
function Backgroundblur-Portrait-Playlist($devPowStat, $token, $SPId) 
{   
    $startTime = Get-Date    
    $ErrorActionPreference='Stop'
    $scenarioName = "$devPowStat\BackgroundBlurPortrait"
    $logFile = "$devPowStat-BackgroundBlurPortrait.txt"
    ...
    try
	{
        ...
        #Toggling Portrait blur effect on
        Write-Output "Entering ToggleAIEffectsInSettingsApp function to toggle Portrait blur effect on"
        ToggleAIEffectsInSettingsApp -AFVal "Off" -PLVal "Off" -BBVal "On" -BSVal "False" -BPVal "True" `
